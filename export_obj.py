...
def write_file(filepath, objects, scene,
    ...
    for ob_main in objects:
        ...
        for ob, ob_mat in obs:
            ...
            reversed = 1
            if (ob_mat.to_scale().x < 0 or ob_mat.to_scale().y < 0 or ob_mat.to_scale().z < 0):
                reversed = -1
                print(ob.name, 'is reversed!')

            if EXPORT_UV:
                faceuv = len(me.uv_textures) > 0
                if faceuv:
                    uv_texture = me.tessface_uv_textures.active.data[:]
                    #uv_layer = me.tessface_uv_layers.active.data[:]
            else:
                faceuv = False
            ...
            if faceuv:
                ...
                for f, f_index in face_index_pairs:
                    ...
                    for j, v_idx in enumerate(f.vertices):
                        uv = uv_texture[f_index].uv[j]
            ...
            if EXPORT_NORMALS:
                for f, f_index in face_index_pairs:
                    if f.use_smooth:
                        ...
                        noKey = veckey3d(f.normal * reversed)
                        if noKey not in globalNormals:
                            globalNormals[noKey] = totno
                            totno += 1
                            fw('vn %.6f %.6f %.6f\n' % noKey)
            ...
            for f, f_index in face_index_pairs:
                ...
                for f_it in f_iter:
                  f_v = [(vi, me_verts[f.vertices[vi]]) for vi in f_it]
                  fw('f')
                  if faceuv:
                    if EXPORT_NORMALS:
                        if f_smooth:  # Smoothed, use vertex normals
                            ...
                            no = globalNormals[veckey3d(f.normal * reversed)]
                            for vi, v in f_v:
                                fw(" %d/%d/%d" %
                                           (v.index + totverts,
                                            totuvco + uv_face_mapping[f_index][vi],
